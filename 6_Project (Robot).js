 ...
 * Persistent group
 * 
 * Most in-built data structures in a standard JavaScript aren’t very well suited for persistent use. 
 * Arrays have slice and concat methods, 
 ...
 * 
 * Why do you need only one PGroup.empty value, 
 * rather than having a function that creates a new, empty map every time?
 * 
 * Hint:
 * The most convenient way to represent the set of member values is 
 * still as an array since arrays are easy to copy.
 * 
 * When a value is added to the group, 
 * you can create a new group with a copy of the original array that has the value added 
 ...
 * 
 * You need only one empty instance because all empty groups are the same and 
 * instances of the class don’t change. You can create many different groups from 
